function [K,M,dKddesign,dMddesign] = get_system_matrices(const)
    ...
    for ele_idx_x = 1:N_ele_x
        for ele_idx_y = 1:N_ele_y
            ...
            row_idxs(start_pointer:end_pointer) = reshape(global_idxs_mat,N_dof_per_element^2,1);
            col_idxs(start_pointer:end_pointer) = reshape(global_idxs_mat',N_dof_per_element^2,1);
            value_K(start_pointer:end_pointer) = reshape(k_ele,N_dof_per_element^2,1);
            value_M(start_pointer:end_pointer) = reshape(m_ele,N_dof_per_element^2,1);
