function wavevectors = create_IBZ_wavevectors(N_k,a,symmetry_type,num_tesselations)
    ...
    if strcmp(symmetry_type,'omit')
        [X,Y] = meshgrid(linspace(-pi/a,pi/a,N_k),linspace(-pi/a,pi/a,N_k)); % a square centered at the origin of side length 2*pi/a
        k_x = X(true(size(X))); k_y = Y(true(size(Y))); % rect
%         IBZ_shape = 'square';
    elseif strcmp(symmetry_type,'none')
        [X,Y] = meshgrid(linspace(-pi/a,pi/a,N_k),linspace(0,pi/a,N_k)); % true asymmetric IBZ - note that this IBZ can be rotated arbitrarily!
        k_x = X(true(size(X))); k_y = Y(true(size(Y))); % rect
%         IBZ_shape = 'rectangle';
    elseif strcmp(symmetry_type,'p4mm')
        [X,Y] = meshgrid(linspace(0,pi/a,N_k),linspace(0,pi/a,N_k)); % these are more points than we need, so we chop it with triu
