        ...
        if const.isComputeGroupVelocity
            for wv_comp_idx = 1:2
                for eig_idx = 1:const.N_eig
                    ...
                    imag_tol = 1e-6;
%                     if abs(imag(cg(k_idx,wv_comp_idx,eig_idx))/real(cg(k_idx,wv_comp_idx,eig_idx))) > imag_tol
...
%                                     dAddesign(:,:,wv_comp_idx,i,j,k) = d2Krddesigndwavevector(:,:,wv_comp_idx,i,j,k) - 2*omega*dfrddesign(k_idx,eig_idx,i,j,k) - omega^2*d2Mrddesigndwavevector(:,:,wv_comp_idx,i,j,k);
                                    dAdtheta = - 2*omega*domegadtheta - omega^2*d2Mrdthetadgamma;
                                end
...
%                                 term1 = -1/(2*omega^2) * dfrddesign(k_idx,eig_idx,i,j,k) * u'*A*u;
%                                 term2 = 1/(2*omega) * duddesign'*A*u;
%                                 term3 = 1/(2*omega) * u'*dAddesign(:,:,wv_comp_idx,i,j,k)*u;
