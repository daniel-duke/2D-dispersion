function k_ele = get_element_stiffness_VEC(E,nu,t,const)
...
% if const.Nod_dof == 2 %Plane strain PSV
%     % dof are [ u1 v1 u2 v2 u3 v3 u4 v4 ] (indexing starts with lower left
%     k_ele = (1/48)*E.*t./(1-nu.^2).*...
%         [(-32.*nu.^2 + 56.*nu - 24)./(-1 + 2.*nu), (-6 + 6.*nu)./(-1 + 2.*nu), ...
%         (8.*nu.^2 - 20.*nu + 12)./(-1 + 2.*nu), (24.*nu.^2 - 30.*nu + 6)./(-1 + 2.*nu),...
%         (16.*nu.^2 - 28.*nu + 12)./(-1 + 2.*nu), (6 - 6.*nu)./(-1 + 2.*nu),...
%         8.*nu.*(-1 + nu)./(-1 + 2.*nu), (-24.*nu.^2 + 30.*nu - 6)./(-1 + 2.*nu),...
%         (-6 + 6.*nu)./(-1 + 2.*nu), (-32.*nu.^2 + 56.*nu - 24)./(-1 + 2.*nu),...
%         (-24.*nu.^2 + 30.*nu - 6)./(-1 + 2.*nu), 8.*nu.*(-1 + nu)./(-1 + 2.*nu),...
%         (6 - 6.*nu)./(-1 + 2.*nu), (16.*nu.^2 - 28.*nu + 12)./(-1 + 2.*nu), ...
%         (24.*nu.^2 - 30.*nu + 6)./(-1 + 2.*nu), (8.*nu.^2 - 20.*nu + 12)./(-1 + 2.*nu),...
%         (8.*nu.^2 - 20.*nu + 12)./(-1 + 2.*nu), (-24.*nu.^2 + 30.*nu - 6)./(-1 + 2.*nu),...
%         (-32.*nu.^2 + 56.*nu - 24)./(-1 + 2.*nu), (6 - 6.*nu)./(-1 + 2.*nu),...
%         8.*nu.*(-1 + nu)./(-1 + 2.*nu), (24.*nu.^2 - 30.*nu + 6)./(-1 + 2.*nu),...
%         (16.*nu.^2 - 28.*nu + 12)./(-1 + 2.*nu), (-6 + 6.*nu)./(-1 + 2.*nu),...
...
elseif const.Nod_dof == 1 % SH (out-of-plane shear)
    k_ele = E.*t./12.*...
        [4./(1 + nu), -1./(1 + nu), -2./(1 + nu), -1./(1 + nu),...
        -1./(1 + nu), 4./(1 + nu), -1./(1 + nu), -2./(1 + nu), ...
        -2./(1 + nu), -1./(1 + nu), 4./(1 + nu), -1./(1 + nu), ...
        -1./(1 + nu), -2./(1 + nu), -1./(1 + nu), 4./(1 + nu)];
