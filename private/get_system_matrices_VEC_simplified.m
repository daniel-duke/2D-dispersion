function [K,M] = get_system_matrices_VEC_simplified(const)
    ...
    edofVec = reshape(2*nodenrs(1:end-1,1:end-1)-1,N_ele_x*N_ele_y,1); % Gives the degree of freedom (global) of the horizontal displacement of the upper left node for each element
    edofMat = repmat(edofVec,1,8)+repmat([2 3 2*(N_ele_x+1)+[2 3 0 1] 0 1],N_ele_x*N_ele_y,1); % For each element, each row of the matrix gives the global dof label corresponding to the local dof indicated by the column
